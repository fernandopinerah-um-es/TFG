"""
=====================================================================
 SCRIPT 2b: MÉTRICAS IR CLÁSICAS PARA EL PIPELINE CON RE-RANKING
=====================================================================

Versión espejo del script 2 que opera sobre los resultados del
pipeline con re-ranking (script 5). La lógica de cálculo es idéntica:
mismas métricas (P@K, R@K, MRR, NDCG@K), mismo criterio fuzzy,
mismo denominador empírico de Recall.

La única diferencia es que los chunks evaluados son los que llegaron
al top-K final TRAS el re-ranking, no los del retriever base. Esto
permite cuantificar exactamente qué aporta el re-ranker en términos
de IR clásico.

Para comparar antes vs después del re-ranker:
    - antes: metricas_ir.json (output script 2)
    - después: metricas_ir_rerank.json (este script)

Input:
    - resultados_rerank.json (output script 5)
    - dataset_test.json (macro-secciones)
    - Elasticsearch (para escanear chunks por documento)
    - cache_denominadores_recall.json (reutiliza el del script 2)

Output:
    - metricas_ir_rerank.json
    - metricas_ir_rerank_resumen.xlsx
"""
...
ARCHIVO_SALIDA_JSON = 'metricas_ir_rerank.json'
ARCHIVO_SALIDA_EXCEL = 'metricas_ir_rerank_resumen.xlsx'
# Reutiliza el mismo cache que el script 2 original: el universo de chunks
# relevantes por (pregunta × chunking) no cambia con re-ranking, solo cambia
# qué chunks llegan al top-K final.
