"""
=====================================================================
 SCRIPT 3b: EVALUACIÓN ESPECÍFICA DE PREGUNTAS NEGATIVAS
=====================================================================

Las preguntas Negativas son consultas fuera del alcance del corpus
(p. ej. preguntas inventadas, off-topic, o sobre información que no
existe en los protocolos). El comportamiento correcto del sistema
RAG ante una Negativa NO es generar una respuesta plausible, sino
RECONOCER que no tiene información suficiente y declinar.

Por esta razón las métricas estándar de RAGAS no son aplicables
(no hay contextos relevantes ni respuesta de referencia sustantiva).
Se evalúan con dos métricas binarias específicas:

    1. NEGATIVE ACCEPTANCE RATE (NAR):
       % de Negativas en las que el sistema declinó correctamente
       reconociendo la falta de información.

    2. HALLUCINATION RATE (HR):
       % de Negativas en las que el sistema inventó una respuesta
       sustantiva en lugar de declinar (= 1 - NAR).

JUEZ:
    Gemini 2.5 Flash con prompt cerrado SÍ/NO sobre la respuesta
    generada.
...
Output:
    - metricas_negativas.json
    - metricas_negativas_resumen.xlsx
"""
...
ARCHIVO_SALIDA_JSON = 'metricas_negativas.json'
ARCHIVO_SALIDA_EXCEL = 'metricas_negativas_resumen.xlsx'
