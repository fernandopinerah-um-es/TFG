"""
=====================================================================
 SCRIPT 3c: MÉTRICAS RAGAS PARA EL PIPELINE CON RE-RANKING
=====================================================================

Versión espejo del script 3 que opera sobre los resultados del
pipeline con re-ranking (script 5). La lógica de cálculo es idéntica:
mismas 5 métricas RAGAS, mismo juez (Gemini 2.5 Flash), mismos
embeddings neutros.

La única diferencia es que evalúa las respuestas generadas a partir
de los chunks RE-RANKEADOS, no los del retriever base. Esto permite
comparar exactamente qué aporta el re-ranker en términos de RAGAS.

NOTA: las top-3 combinaciones evaluadas son todas A/B/C (ninguna D),
por lo que el bloque interno de "solo generación para D_jerarquico"
no procesará nada en este caso. El código se mantiene intacto por
robustez.

Para comparar antes vs después del re-ranker:
    - antes: metricas_ragas.json (output script 3)
    - después: metricas_ragas_rerank.json (este script)
...
Output:
    - metricas_ragas_rerank.json
    - metricas_ragas_rerank_resumen.xlsx
"""
...
ARCHIVO_SALIDA_JSON = 'metricas_ragas_rerank.json'
ARCHIVO_SALIDA_EXCEL = 'metricas_ragas_rerank_resumen.xlsx'
